...
class BinanceTestnetAPIError(RuntimeError):
    def __init__(self, status: int, code: int | None, message: str, path: str) -> None:
        ...
        self.path = path
        super().__init__(
            f"Binance Testnet API error on {path}: HTTP {status}, "
...
class TestnetExecutionClient:
    ...
    def _signed_request(
        ...
        for attempt in range(self.max_retries):
            ...
            try:
            ...
            except BinanceTestnetAPIError as exc:
                last_error = exc
                ...
                time.sleep(self.retry_sleep_seconds * (attempt + 1))
        raise RuntimeError(
            f"Testnet API request failed after {self.max_retries} retries: {path}"
        ) from last_error
    ...
    def _sync_server_time(self) -> None:
        ...
        if not isinstance(payload, dict) or "serverTime" not in payload:
            raise RuntimeError("Invalid Binance Testnet server time response.")
        local_midpoint = (local_before + local_after) // 2
        self._clock_offset_ms = int(payload["serverTime"]) - local_midpoint
        logger.warning(
            "Synchronized Binance Testnet clock offset: %dms",
...
def _api_error_from_http_error(exc: HTTPError, path: str) -> BinanceTestnetAPIError:
    code: int | None = None
    ...
    try:
        ...
        pass
    return BinanceTestnetAPIError(exc.code, code, message, path)
...
BinanceFuturesTestnetClient = TestnetExecutionClient
