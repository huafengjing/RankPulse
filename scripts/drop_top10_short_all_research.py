...
OUT = ROOT / "outputs" / "drop_top10_short_all"
INTERVAL = "5m"
INTERVAL_MINUTES = 5
LOOKBACK_DAYS = 180
COOLDOWN_DAYS = 5
...
def drop_bucket(value: float) -> str:
    if pd.isna(value):
        return "unknown"
    if value < 0.05:
        return "<5%"
            ...
            rows.append(
                {
                    ...
                    "rolling_24h_drop_bucket": drop_bucket(drop),
                    "price_at_signal": float(row["close"]),
                    "quote_volume_at_signal": float(row.get("quote_volume", 0.0)),
                    "quote_volume_24h": float(row.get("quote_volume_24h_sum", 0.0)),
                }
            )
            signal_id += 1
            previous_time = now
...
def extract_features(signals: pd.DataFrame, klines: pd.DataFrame, rankings: pd.DataFrame) -> pd.DataFrame:
    ...
    rank_symbol_map = {s: g.reset_index(drop=True) for s, g in rankings.groupby("symbol", sort=False)}
    ...
    for _, sig in signals.iterrows():
        ...
        symbol_rankings = rank_symbol_map[symbol]
        old30 = asof_row(symbol_rankings, signal_time - 30 * 60_000)
        old60 = asof_row(symbol_rankings, signal_time - 60 * 60_000)
        hist = symbol_rankings[symbol_rankings["open_time"] < signal_time]
        ...
        features = {
            ...
            "rolling_24h_drop_bucket": drop_bucket(float(sig["rolling_24h_drop_at_signal"])),
            ...
            "price_below_4h_ma14": close < float(row["ma14_4h"]) if pd.notna(row["ma14_4h"]) else False,
...
def bucket_analysis(df: pd.DataFrame) -> pd.DataFrame:
    specs = {
        "rolling_24h_drop_at_signal": ([0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.40, np.inf], ["5%-10%", "10%-15%", "15%-20%", "20%-25%", "25%-30%", "30%-40%", ">40%"]),
    ...
    for feature, (bins, labels) in specs.items():
        ...
        for (drop_b, bucket), g in tmp.groupby(["rolling_24h_drop_bucket", "_bucket"], observed=False):
            if g.empty:
                continue
            pnl = g["simple_pnl"]
            rows.append(
...
def drop_bucket_summary(df: pd.DataFrame) -> pd.DataFrame:
    order = ["<5%", "5%-10%", "10%-15%", "15%-20%", "20%-25%", "25%-30%", "30%-40%", ">40%"]
    rows = []
    for bucket in order:
        g = df[df["rolling_24h_drop_bucket"].eq(bucket)]
        if g.empty:
            continue
        pnl = g["simple_pnl"]
        rows.append(
            {
                ...
                "first_plus10_rate": float(g["hit_plus_10_before_minus_10"].mean()),
                "first_minus10_rate": float(g["hit_minus_10_before_plus_10"].mean()),
                "first_minus15_rate": float(g["hit_minus_15_before_plus_10"].mean()),
                "first_minus20_rate": float(g["hit_minus_20_before_plus_10"].mean()),
                "first_minus30_rate": float(g["hit_minus_30_before_plus_10"].mean()),
                "first_minus40_rate": float(g["hit_minus_40_before_plus_10"].mean()),
                "first_minus50_rate": float(g["hit_minus_50_before_plus_10"].mean()),
                "avg_mfe_down": float(g["max_favorable_down_120h"].mean()),
                "avg_mae_up": float(g["max_adverse_up_120h"].mean()),
                "median_mfe_down": float(g["max_favorable_down_120h"].median()),
                "median_mae_up": float(g["max_adverse_up_120h"].median()),
                "simple_short_pf": profit_factor(pnl),
                "simple_short_total_pnl": float(pnl.sum()),
                "sample_flag": "valid" if len(g) >= 50 else "sample_too_small",
            }
        )
    return pd.DataFrame(rows)
...
def evaluate_entry_models(base: pd.DataFrame, kmap: dict[str, pd.DataFrame], rankings: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, pd.DataFrame]]:
    ...
    for model in models:
        ...
        trades = make_backtest_trades(events, kmap, "base_all_drop_top10", model, "Exit C") if not events.empty else pd.DataFrame()
        summaries.append(
            {
                "model_name": model,
                "base_filter": "base_all_drop_top10",
...
def robustness(summary: pd.DataFrame, trades: pd.DataFrame, base_first_plus10_rate: float) -> pd.DataFrame:
    rows = []
    if trades.empty:
        return pd.DataFrame()
    ...
    for keys, g in grouped:
        ...
        overfit = len(v) < 30 or (train_pf >= 1.15 and (val_pf <= 1 or val_plus >= base_first_plus10_rate))
        rows.append(
            {
                "rule_name": keys[0],
                "entry_model": keys[1],
                ...
                "conclusion": "valid" if (len(v) >= 30 and val_pf > 1 and val_plus < base_first_plus10_rate) else "not_valid",
            }
        )
    return pd.DataFrame(rows)
...
def main() -> None:
    ...
    drop_bucket_summary(df).to_csv(OUT / "drop_bucket_summary.csv", index=False)
    ...
    exit_c_all = make_backtest_trades(immediate_events, kmap, "base_all_drop_top10", "Immediate", "Exit C")
    filter_defs = [
        ...
        ("Filter K", "10% <= rolling_24h_drop_at_signal <= 20%", lambda x: x["rolling_24h_drop_at_signal"].between(0.10, 0.20)),
        ("Filter L", "15% <= rolling_24h_drop_at_signal <= 25%", lambda x: x["rolling_24h_drop_at_signal"].between(0.15, 0.25)),
        ("Filter M", "rolling_24h_drop_at_signal <= 25%", lambda x: x["rolling_24h_drop_at_signal"] <= 0.25),
        ("Filter N", "rolling_24h_drop_at_signal >= 10%", lambda x: x["rolling_24h_drop_at_signal"] >= 0.10),
    ]
    single_rows = []
    ...
    combo_defs = [
        ("Combo 1", "drop 10-20 + vol 1.2-5", lambda x: x["rolling_24h_drop_at_signal"].between(0.10, 0.20) & x["volume_1h_vs_24h_avg"].between(1.2, 5)),
        ("Combo 2", "drop 15-25 + vol 1.2-5", lambda x: x["rolling_24h_drop_at_signal"].between(0.15, 0.25) & x["volume_1h_vs_24h_avg"].between(1.2, 5)),
        ("Combo 3", "drop 10-20 + rebound30 <=5", lambda x: x["rolling_24h_drop_at_signal"].between(0.10, 0.20) & (x["rebound_from_local_low_30m_pct"] <= 0.05)),
        ("Combo 4", "drop 15-25 + rebound30 <=5", lambda x: x["rolling_24h_drop_at_signal"].between(0.15, 0.25) & (x["rebound_from_local_low_30m_pct"] <= 0.05)),
        ("Combo 5", "drop 10-20 + vol 1.2-5 + lower wick <=40", lambda x: x["rolling_24h_drop_at_signal"].between(0.10, 0.20) & x["volume_1h_vs_24h_avg"].between(1.2, 5) & (x["signal_candle_lower_wick_pct"] <= 0.40)),
        ("Combo 6", "drop 10-20 + vol 1.2-5 + btc1h <=0", lambda x: x["rolling_24h_drop_at_signal"].between(0.10, 0.20) & x["volume_1h_vs_24h_avg"].between(1.2, 5) & (x["btc_return_1h"] <= 0)),
        ("Combo 7", "drop 15-25 + vol 1.2-5 + dist ema20 <=8", lambda x: x["rolling_24h_drop_at_signal"].between(0.15, 0.25) & x["volume_1h_vs_24h_avg"].between(1.2, 5) & (x["distance_to_5m_ema20_pct"].abs() <= 0.08)),
        ("Combo 8", "drop <=25 + vol <=5 + rebound30 <=5", lambda x: (x["rolling_24h_drop_at_signal"] <= 0.25) & (x["volume_1h_vs_24h_avg"] <= 5) & (x["rebound_from_local_low_30m_pct"] <= 0.05)),
        ("Combo 9", "drop 10-25 + rank>=4 + lower wick <=40", lambda x: x["rolling_24h_drop_at_signal"].between(0.10, 0.25) & (x["rank_at_signal"] >= 4) & (x["signal_candle_lower_wick_pct"] <= 0.40)),
        ("Combo 10", "drop 10-20 + not drop3d + not gain5d", lambda x: x["rolling_24h_drop_at_signal"].between(0.10, 0.20) & (~x["was_in_drop_top10_last_3d"].astype(bool)) & (~x["was_in_gain_top10_last_5d"].astype(bool))),
    ]
    combo_rows = []
    combo_id_sets = {"base_all_drop_top10": set(df["signal_id"])}
    ...
    trade_frames = []
    rules_to_test = {"base_all_drop_top10": set(df["signal_id"])}
    ...
    base_first_plus10_rate = float(df["hit_plus_10_before_minus_10"].mean()) if len(df) else np.nan
    robustness(backtest_summary, trades_all, base_first_plus10_rate).to_csv(OUT / "robustness_check.csv", index=False)

    best_filters = single_summary.sort_values(["remaining_first_plus10_rate", "remaining_count"], ascending=[True, False]).head(5)
    valid_bt = backtest_summary[(backtest_summary["trade_count"] >= 100) & (backtest_summary["profit_factor"] >= 1.15)].sort_values("profit_factor", ascending=False)
    valid_rob = pd.read_csv(OUT / "robustness_check.csv")
    report = [
        "# Drop Top10 Short All-Signal Research",
        "",
        ...
        f"Base sample: first drop Top10 within {COOLDOWN_DAYS}d, no trigger-drop prefilter, next 5m open short entry.",
        ...
        "",
        "## Drop Bucket Summary",
        dataframe_to_markdown(pd.read_csv(OUT / "drop_bucket_summary.csv")),
        "",
        "## Best Single Filters By Lower First +10%",
        dataframe_to_markdown(best_filters),
        "",
        "## Backtest Candidates PF >= 1.15 And Count >= 100",
        dataframe_to_markdown(valid_bt.head(20)) if not valid_bt.empty else "No candidate met PF >= 1.15 with count >= 100.",
        "",
        "## Validation",
        ...
        "2. drop_bucket_summary.csv compares all trigger-drop buckets before filters.",
        ...
        "10. Do not enter simulation unless validation PF > 1, first +10% below full baseline, and monthly performance is not concentrated.",
