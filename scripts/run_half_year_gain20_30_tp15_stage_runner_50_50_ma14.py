...
FEATURES = Path("outputs/half_year_gain20_30_signals/signals_gain20_30_first_top10.csv")
OUT = Path("outputs/half_year_gain20_30_tp15_stage_runner_50_50_ma14")
...
def run_trade(sig: pd.Series, group: pd.DataFrame) -> dict | None:
    ...
    return {
        ...
        "rank_at_signal": sig["rank"],
        "rolling_24h_gain_at_signal": sig["rolling_24h_change_pct"],
        "tp1_hit": tp1_hit,
    ...
    out = pd.DataFrame(
        {
            ...
            "爆仓率": summary["liquidation_rate"].map(lambda x: f"{x * 100:.2f}%"),
